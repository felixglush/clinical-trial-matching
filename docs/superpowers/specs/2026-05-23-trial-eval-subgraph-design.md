...
2. **Score mechanism plausibility by the right source for the channel.** Repurposing-channel trials inherit the upstream TxGNN `predIndication` score, with `mechanismRationale` filled from a small template (no LLM call — `find-repurposing-candidates` already produced the canonical sentence and we just reformat it with patient mechanism context). Strategy-channel trials — surfaced by lexical CT.gov keyword match, with no biology vetted upstream — get the full `pathBetween` + LLM scoring pass. See *Channel-aware behavior* in `mechanism-plausibility` below.
3. **Surface citations as artifacts, not as a score pillar.** `literature-support` still runs (two PubMed attempts max; broaden on retry; union citations across attempts, deduped by pmid), and the resulting `Citation[]` lives on the `TrialMatch` for the clinician brief. But citation *count* is not a score input: nearly every CT.gov drug-condition combination has *some* PubMed footprint, so counting hits mostly measures whether the combo has ever been studied (which it has — that's why CT.gov has a trial for it), not whether the evidence supports this patient. The score is decided by eligibility + mechanism; literature accompanies the match as supporting evidence the reviewer can follow.
4. **Compose a reproducible sort key, not a clinical verdict.** The `TrialMatch.score` is a single 0–100 number, but it's a *sort key* — the audit surface is the per-pillar fields already on `TrialMatch` (`eligibility.overall`, `mechanismScore`, `literatureSupport`, `concerns`). UI / `rank-and-synthesize` should present those pillars as first-class badges; the combined score is what we sort by, not the clinical judgment we hand to the reviewer. The formula gates on eligibility to prevent absurd orderings (an ineligible patient cannot score above 25 regardless of biology).
...
| `score` semantics | Sort key, not a clinical verdict. Per-pillar fields (`eligibility.overall`, `mechanismScore`, `literatureSupport`, `concerns`) are the audit surface. | A single number invites being read as a recommendation. The clinician needs the breakdown; ranking just needs a deterministic order. |
...
| Score formula | Eligibility-gated weighted sum: `ineligible → 0`; `likely_ineligible → min(25, weightedSum)`; otherwise `weightedSum = 0.6·E + 0.4·M` | Two-pillar weighted sum. Eligibility weight stays higher than mechanism because mismatched eligibility is a hard blocker; mechanism is the patient-specific biological signal. The gate enforces that an unenrollable patient can't outrank an enrollable one regardless of biology. **Literature is not a score input** — see "Literature as artifact" below. |
...
| Mechanism sub-score | Channel-dependent: see *Channel-aware behavior* in `mechanism-plausibility`. Repurposing → TxGNN `predIndication × 100` (no LLM). Strategy → LLM-produced 0–100. Null → 50 (neutral) with a concern. | The right source for each channel; avoids re-deriving TxGNN's signal with a thinner LLM-with-KG-paths pass. |
| Literature as artifact | `literature-support` runs and `TrialMatch.literatureSupport: Citation[]` is populated for the clinician brief. `synthesize-match`'s narrate LLM also sees the citation titles (first 3) when writing `summary`. Citation count does **not** feed the score formula. | Counting CT.gov drug-condition citations measures "has this combo ever been studied" (yes — that's why CT.gov has the trial), not "is the evidence relevant to this patient." Keep the data; don't pretend the count is a score. |
...
| `mechanism-plausibility` channel split | Repurposing-channel trials skip the LLM entirely: score is `TxGNN predIndication × 100`; rationale is templated from the source `RepurposingCandidate.rationale` augmented with patient mechanism context. Strategy-channel trials run the full `pathBetween` + LLM scoring + LLM rationale pass. Both-channel trials take the repurposing path (TxGNN score is the authoritative source for that drug-disease pair). | TxGNN already ranked the drug for the patient's MONDO upstream — re-scoring with 3 KG paths and an LLM duplicates work and risks signal drift. `find-repurposing-candidates` also already produced the canonical one-line rationale; an LLM call here would be a third generator for the same sentence. Strategy-channel matches are lexical CT.gov keyword hits with no upstream biology check, so they need the full pass. |
...
**Path A — repurposing-channel (TxGNN score + templated rationale, NO LLM):**
...
2. `mechanismScore = round(sourceRC.predIndication * 100)`.
3. `mechanismRationale` is templated, not LLM-generated:
   ```ts
   // formatPathSummary picks the intermediate node names from the
   // supporting path (skipping drug and disease) and joins them with " / ".
   // Example: "EGFR / ERBB signaling pathway"
   const pathSummary = sourceRC.supportingPaths.length > 0
     ? `via ${formatPathSummary(sourceRC.supportingPaths[0])}`
     : "no TxGNN explanation path available";
   mechanismRationale =
     `TxGNN predicted ${sourceRC.drug.name} for ${sourceRC.originalIndications.join(", ")} ` +
     `(indication ${predIndication.toFixed(2)}); ${pathSummary}.`;
   ```
4. **Why no LLM in Path A.** Three rationale generators already exist for this drug-disease relationship: (a) `find-repurposing-candidates.ts:165` produces a templated one-liner on `RepurposingCandidate.rationale`; (b) the TxGNN explanation path is committed as data; (c) `synthesize-match`'s narrate LLM composes the user-facing `summary` reading all the structured signals. A fourth LLM call here re-narrates content the synthesize-match LLM already covers. Saves one LLM call per repurposing-channel candidate without quality loss.
...
> **Note on path provenance (Path A).** `RepurposingCandidate.supportingPaths` is already a `KGPath[]` — `find-repurposing-candidates` populates it via `tools/txgnn.ts::lookupExplanation()`, and the TxGNN explanation data (`apps/agent/src/data/txgnn-explanations.json`) is committed in `KGPathSchema` form (types pre-normalized to `gene_protein`, edges already directional with PrimeKG-verbatim relation strings, uniformly `drug → gene → process → disease` 3-hop). The Path A template reads the path's intermediate node names directly; no re-query, no Cypher fallback, no shape transformation.

**Error handling:**

- Path A: there is no LLM call to fail. Missing `supportingPaths` triggers the "no TxGNN explanation path available" branch of the template; score remains TxGNN-sourced.
...
Weights are named constants in the node module (`WEIGHT_ELIGIBILITY = 0.6`, `WEIGHT_MECHANISM = 0.4`, `LIKELY_INELIGIBLE_CAP = 25`) — easy to tune.

**Why no literature in the formula.** Citation count saturates fast: any drug-condition combination CT.gov is enrolling for almost certainly has *some* PubMed footprint, so the count mostly measures "this combo has been studied" — which is already true by virtue of the trial existing. Per Step 2 below, `synthesize-match`'s narrate LLM does see the citation titles when writing `summary`, and `literatureSupport` lives on the final `TrialMatch` for the clinician brief; we just don't pretend the count is a score input.

**Why the gate.** A pure weighted sum makes `(eligibility=ineligible=0, mechanism=80)` score `0 + 32 = 32`, which reads as "marginal." That's wrong: an ineligible patient can't enroll, full stop, so the trial should not outrank an `eligible` candidate with weaker biology. The gate enforces that — eligibility is *permission to be scored*, not just one of two weights. This is also why the `score` field is documented as a sort key, not a clinical verdict: clinicians read `eligibility.overall`, `mechanismScore`, `literatureSupport`, and `concerns` directly off the `TrialMatch`; the score just orders the list.
...
- Prompt receives:
  - patient summary
  - trial summary
  - the two sub-scores (eligibility, mechanism) + the total
  ...
  - citation count + first 3 citation titles (as supporting evidence the summary can reference — but the LLM is told the count does NOT affect the score)
- Structured output:
  ```ts
  ...
  ```
- Prompt instructs: write a 2–3 sentence summary describing the match; populate `concerns[]` with explicit red flags (contraindication present, inclusion criteria not met, no mechanism path, no PubMed evidence found).
...
Per subgraph invocation, the call shape is (Path A is the repurposing-channel case; Path B is the strategy-channel case):

| Step | LLM calls | Neo4j calls | PubMed calls |
|---|---|---|---|
| `eligibility-check` | 1 | 1 | 0 |
| `mechanism-plausibility` Path A | **0** | 0 | 0 |
| `mechanism-plausibility` Path B | 1 | up to 5 × 3 = 15 (per intervention × per mechanism, parallel) | 0 |
| `literature-support` (×2 max) | 0 | 0 | 1 (×2 max) |
| `synthesize-match` | 1 | 0 | 0 |
| **Per-subgraph total (Path A)** | **2** | **1** | **up to 2** |
| **Per-subgraph total (Path B)** | **3** | **up to ~16** | **up to 2** |

Parent graph fans out up to `MAX_EVALUATIONS = 5` subgraphs in parallel. Peak load is therefore:

- **LLM:** sequential within a subgraph, so peak is 5 in-flight (one per fan-out branch) regardless of which channel. Path A subgraphs make 2 LLM calls total, Path B make 3. Haiku via OpenRouter handles this without throttling.
- **Neo4j:** 80 in-flight queries at peak in the worst case (5 × Path B's 16). `neo4j-driver`'s default connection pool is 100 — fits.
...
| `mechanism-plausibility` Path A (no LLM call) | Cannot fail in the LLM sense. Missing `RepurposingCandidate.supportingPaths` triggers the "no TxGNN explanation path available" branch of the template; score is still TxGNN-sourced |
| LLM API failure in `mechanism-plausibility` Path B | `mechanismScore = null` → maps to 50 + adds "mechanism evaluation unavailable" concern |
...
| Both literature attempts return 0 citations | `literatureSupport: []`; narrate LLM is told there are no citations; `concerns` includes "no PubMed evidence found"; score is unaffected (literature isn't a formula input) |
...
5. **PubMed query construction is hand-crafted, not LLM-generated.** Trade is reproducibility vs. recall — a slightly more clever query (synonyms, MeSH terms, year limits) might surface better citations, but we don't want a per-trial LLM call in the literature step. The recall question is now less material to ranking (citation count no longer feeds the score) — but the citations are still part of the clinician brief, so a better query still helps the reviewer.
...
7. **No abstract retrieval in v1.** `Citation.abstractExcerpt` will be undefined; `synthesize-match`'s prompt only sees titles. If the narrate LLM consistently writes "no abstract context available" in its summaries, add the efetch step.
8. **Score weights are guesses.** 0.6 / 0.4 between eligibility and mechanism — eyeball the first 10 runs; if the rank order disagrees with clinician intuition systematically, reweight. Constants are named for easy tuning.
...
- **`subgraphs/trial-eval/nodes/mechanism-plausibility.test.ts`** — cover both channel paths:
  - Path A (repurposing-only): assert `mechanismScore = round(predIndication × 100)`; assert `mechanismRationale` is templated and includes the drug name + indication score + intermediate node names from `supportingPaths` (or "no TxGNN explanation path available" when missing); **assert the LLM is never invoked** (no narrate call in Path A).
  ...
  - Both-channel: assert Path A taken (TxGNN score wins); LLM never invoked.
...
- **`subgraphs/trial-eval/nodes/synthesize-match.test.ts`** — fixtures covering the score formula + eligibility gate (literature is NOT a formula input):
  - (a) `eligible` + mechanism 80 → weightedSum = `0.6·100 + 0.4·80 = 92` → score 92 (no gate). Citation count does not change the result.
  - (b) `likely_ineligible` + null mechanism (50) → weightedSum = `0.6·25 + 0.4·50 = 35`; gate caps at 25 → score 25.
  - (c) `ineligible` + mechanism 90 → weightedSum = `0.6·0 + 0.4·90 = 36` → gate forces score 0; concerns include "patient ineligible".
  - (d) `likely_eligible` + null mechanism (50) → weightedSum = `0.6·75 + 0.4·50 = 65` → score 65.
  - (e) Citation count invariance: same inputs as (a) with 0 vs 10 citations → identical score.
  - (f) Repurposing-channel candidate → `repurposingRationale` populated from `state.repurposingCandidates`; templated `summary` matches the spec.
